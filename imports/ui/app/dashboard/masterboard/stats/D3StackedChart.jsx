...
class D3StackedChart extends Component {
  createChart(_self) {
    ...
    this.xScale = d3.scale.ordinal()
        .rangeRoundBands([0, this.w], 0.35)
        .domain(this.stacked[0].map(d => d.x));
  ...
  render() {
    ...
    if (this.props.children != null) {
      if (Array.isArray(this.props.children)) {
        elements = this.props.children.map((element, i) => _self.createElements(element, i));
      } else {
        elements = this.createElements(this.props.children, 0);
      }
    }

    const bars = _self.stacked.map((data, i) => {
